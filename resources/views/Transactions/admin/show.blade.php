    ...
    <x-dashboard.sidebar>
        ...
        <div class="bg-white grid grid-cols-1 w-full h-auto mt-5 rounded-md p-5">
            ...
            <div class="flex flex-col md:flex-row gap-5">
                <div>
                    ...
                    <input type="datetime-local" name="waktu_awal" class="rounded" value="{{ $transaksi->waktu_awal }}"
                        disabled>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3">
                <div class="max-w-sm mt-3">
                    ...
                    @if ($transaksi->jenis_transaksi == 'ruang_kelas')
                        <input type="text" name="product"
                            value="{{ old('product', $transaksi->ruangkelas->nama_ruangan ?? '-') }}"
                    ...
                    @else
                        <input type="text" name="product"
                            value="{{ old('product', $transaksi->kendaraan->nama_kendaraan ?? '-') }}"
                            class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            disabled>
                    @endif
                </div>
                <div class="max-w-sm mt-3">
                    ...
                    <input type="text" id="total_harga" name="total_harga" readonly
                        class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                        value="Rp {{ number_format($transaksi->total_transaksi, 0, ',', '.') }}">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3">
                <div class="max-w-sm mt-3">
                ...
                <div class="max-w-sm mt-3">
                    ...
                    <input type="text" id="nim" name="nim" readonly
                        class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                        value="{{ $transaksi->user->nim }}">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3">
                <div class="max-w-sm">
                    <label for="bukti_pembayaran"
                        class="block mb-2 text-sm font-medium text-slate-400 dark:text-white mt-3">Bukti
                        Pembayaran</label>
                ...
                <div class="max-w-sm">
                    <label for="bukti_surat" class="block mb-2 text-sm font-medium text-slate-400 dark:text-white mt-3">
                        Bukti Surat
                    </label>
                    @if ($transaksi->bukti_surat)
                        <a href="{{ asset('storage/' . $transaksi->bukti_surat) }}" target="_blank">
                            <img src="{{ asset('storage/' . $transaksi->bukti_surat) }}" alt="Bukti Surat"
                                class="w-24 h-auto">
                        </a>
                    @else
        ...
        <div class="bg-white grid grid-cols-1 w-full h-auto mt-5 rounded-md p-5">
            <div>
                ...
                @if ($transaksi->status == 'dalam_proses')
                    <form id="rejectForm" action="{{ route('admin.transaction.reject', $transaksi->id) }}"
